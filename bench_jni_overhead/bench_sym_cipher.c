...
static int doEncrypt(CpaCySymOpData *opData,
...
static void *cipherThruWorker(void *arg)
{
    ...
    Cpa8U *uSrc     = qaeMemAllocNUMA(DATA_BYTES, 0, 64);
    Cpa8U *uDst     = qaeMemAllocNUMA(DATA_BYTES, 0, 64);
    Cpa8U *uIv      = qaeMemAllocNUMA(AES_IV_LEN, 0, 64);
    Cpa8U *uSrcMeta = metaSize ? qaeMemAllocNUMA(metaSize, 0, 64) : NULL;
    Cpa8U *uDstMeta = metaSize ? qaeMemAllocNUMA(metaSize, 0, 64) : NULL;

    for (int i = 0; i < DATA_BYTES; i++) uSrc[i] = (Cpa8U)(i & 0xFF);
    memset(uIv, 0, AES_IV_LEN);

    CpaFlatBuffer srcFlat = { .dataLenInBytes = DATA_BYTES, .pData = uSrc };
    CpaFlatBuffer dstFlat = { .dataLenInBytes = DATA_BYTES, .pData = uDst };
    CpaBufferList srcList = { .numBuffers = 1, .pBuffers = &srcFlat,
                              .pPrivateMetaData = uSrcMeta };
    CpaBufferList dstList = { .numBuffers = 1, .pBuffers = &dstFlat,
    ...
    opData.pIv                         = uIv;
    opData.ivLenInBytes                = AES_IV_LEN;
    opData.cryptoStartSrcOffsetInBytes = 0;
    opData.messageLenToCipherInBytes   = DATA_BYTES;

    for (int i = 0; i < THRU_ITERS; i++) {
        long long t0 = nowNs();
        doEncrypt(&opData, &srcList, &dstList);
        a->ns[i] = nowNs() - t0;
    }

    qaeMemFreeNUMA((void **)&uSrc);
    qaeMemFreeNUMA((void **)&uDst);
    qaeMemFreeNUMA((void **)&uIv);
...
int main(void)
{
    printf("=== bench_sym_cipher: QAT AES-128-CBC encrypt (%d bytes, pure C) ===\n\n",
           DATA_BYTES);

    if (initQat() != 0) return 1;

    /* AES-128 key (synthetic). */
    static Cpa8U aesKey[AES_KEY_LEN] = {
    ...
    ssd.symOperation                             = CPA_CY_SYM_OP_CIPHER;
    ssd.cipherSetupData.cipherAlgorithm          = CPA_CY_SYM_CIPHER_AES_CBC;
    ssd.cipherSetupData.cipherKeyLenInBytes      = AES_KEY_LEN;
    ssd.cipherSetupData.pCipherKey               = aesKey;
    ssd.cipherSetupData.cipherDirection          = CPA_CY_SYM_CIPHER_DIRECTION_ENCRYPT;
    ...
    /* Determine metadata size for a 1-buffer CpaBufferList (same for all instances). */
    Cpa32U metaSize = 0;
    cpaCyBufferListGetMetaSize(g_cyInst[0], 1, &metaSize);

    /* Allocate USDM buffers. */
    Cpa8U *uSrc      = qaeMemAllocNUMA(DATA_BYTES, 0, 64);
    Cpa8U *uDst      = qaeMemAllocNUMA(DATA_BYTES, 0, 64);
    Cpa8U *uIv       = qaeMemAllocNUMA(AES_IV_LEN, 0, 64);
    Cpa8U *uSrcMeta  = metaSize ? qaeMemAllocNUMA(metaSize, 0, 64) : NULL;
    ...
    /* Fill plaintext and IV. */
    for (int i = 0; i < DATA_BYTES; i++) uSrc[i] = (Cpa8U)(i & 0xFF);
    memset(uIv, 0, AES_IV_LEN);

    /* Set up CpaBufferList structs (stack-allocated metadata arrays OK). */
    CpaFlatBuffer srcFlat = { .dataLenInBytes = DATA_BYTES, .pData = uSrc };
    CpaFlatBuffer dstFlat = { .dataLenInBytes = DATA_BYTES, .pData = uDst };
    ...
    /* Per-operation data. IV must remain valid through the async callback. */
    CpaCySymOpData opData;
    memset(&opData, 0, sizeof(opData));
    opData.sessionCtx               = g_session[0]; /* overwritten per-call by doEncrypt */
    opData.packetType               = CPA_CY_SYM_PACKET_TYPE_FULL;
    opData.pIv                      = uIv;
    opData.ivLenInBytes             = AES_IV_LEN;
    opData.cryptoStartSrcOffsetInBytes = 0;
    ...
    for (int i = 0; i < WARMUP_ITERS; i++) {
        if (doEncrypt(&opData, &srcList, &dstList) != 0) {
    ...
    for (int t = 0; t < THRU_THREADS; t++)
        pthread_create(&thruTh[t], NULL, cipherThruWorker, &thruA[t]);
    for (int t = 0; t < THRU_THREADS; t++)
