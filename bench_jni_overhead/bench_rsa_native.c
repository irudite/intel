 ...
 * bench_rsa_native.c
 *
 * Pure C latency/throughput benchmark for QAT RSA-2048 encrypt.
 * USDM buffers are pre-allocated once; each timed iteration submits
 * the operation and spins on the completion flag.  This isolates the
 * QAT hardware time with no per-call copy or alloc overhead.
 *
 * Compare the output against RsaJniBenchmark.java to measure how much
 * of the JNI path is hardware vs. JNI call overhead + buffer copies.
 *
 * Compile:
 *   gcc -O2 -o bench_rsa_native bench_rsa_native.c \
 *       -I/usr/include/qat -lqat -lusdm -lpthread
 *
 * Run:
 *   sudo ./bench_rsa_native
 */
...
#define LAT_ITERS       5000        /* stored individually for percentiles  */
#define THRU_ITERS      10000       /* ops per thread                       */
#define THRU_THREADS    16          /* concurrent threads to saturate QAT   */
...
/* ---- Test vectors: placeholder 2048-bit key material ---- */
static Cpa8U g_modN[KEY_BYTES];
static Cpa8U g_expE[KEY_BYTES];
static Cpa8U g_plaintext[KEY_BYTES];

static void initTestVectors(void)
{
    memset(g_modN, 0xFF, KEY_BYTES);
    g_modN[0] = 0xC0;                   /* top two bits set — looks valid    */
    g_modN[KEY_BYTES - 1] = 0x01;       /* odd modulus                       */

    memset(g_expE, 0, KEY_BYTES);
    g_expE[KEY_BYTES - 3] = 0x01;       /* e = 65537 = 0x010001 big-endian   */
    g_expE[KEY_BYTES - 1] = 0x01;

    memset(g_plaintext, 0, KEY_BYTES);
    g_plaintext[KEY_BYTES - 1] = 0x42;  /* small value ensures plaintext < N */
    g_plaintext[KEY_BYTES - 2] = 0x41;
}
...
static int initQat(void)
{
    ...
    if (num == 0) { fprintf(stderr, "No CY instances\n"); icp_sal_userStop(); return -1; }
...
/* ---- Single encrypt (pre-allocated USDM) ---- */
static int doEncrypt(CpaCyRsaEncryptOpData *op, CpaFlatBuffer *out)
{
    Completion comp = { .complete = 0, .status = CPA_STATUS_SUCCESS };
    CpaStatus s = cpaCyRsaEncrypt(pickInstance(), rsaCallback, &comp, op, out);
    if (s != CPA_STATUS_SUCCESS) return -1;
    while (!comp.complete) usleep(1);
    return (comp.status == CPA_STATUS_SUCCESS) ? 0 : -1;
}

/* ---- Multi-threaded throughput worker ---- */
static void *rsaEncThruWorker(void *arg)
{
    ThruArg *a = (ThruArg *)arg;

    Cpa8U *uN  = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uE  = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uPt = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uCt = qaeMemAllocNUMA(KEY_BYTES, 0, 64);

    memcpy(uN, g_modN, KEY_BYTES);
    memcpy(uE, g_expE, KEY_BYTES);
    memcpy(uPt, g_plaintext, KEY_BYTES);

    CpaCyRsaPublicKey pubKey;
    memset(&pubKey, 0, sizeof(pubKey));
    pubKey.modulusN.dataLenInBytes        = KEY_BYTES;
    pubKey.modulusN.pData                 = uN;
    pubKey.publicExponentE.dataLenInBytes = KEY_BYTES;
    pubKey.publicExponentE.pData          = uE;

    CpaCyRsaEncryptOpData op;
    memset(&op, 0, sizeof(op));
    op.pPublicKey               = &pubKey;
    op.inputData.dataLenInBytes = KEY_BYTES;
    op.inputData.pData          = uPt;

    CpaFlatBuffer outBuf = { .dataLenInBytes = KEY_BYTES, .pData = uCt };
...
int main(void)
{
    printf("=== bench_rsa_native: QAT RSA-%d encrypt (pure C, USDM pre-allocated) ===\n\n",
           KEY_BYTES * 8);
    ...
    /* Pre-allocate USDM once — no per-call alloc overhead */
    Cpa8U *uN  = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uE  = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uPt = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    Cpa8U *uCt = qaeMemAllocNUMA(KEY_BYTES, 0, 64);
    ...
    for (int i = 0; i < WARMUP_ITERS; i++) {
        if (doEncrypt(&op, &outBuf) != 0) {
            fprintf(stderr, "Warmup failed at iter %d\n", i);
            goto cleanup;
        }
    }

    /* --- Latency benchmark: record every op --- */
    ...
    for (int t = 0; t < THRU_THREADS; t++)
        pthread_create(&thruTh[t], NULL, rsaEncThruWorker, &thruA[t]);
    for (int t = 0; t < THRU_THREADS; t++)
